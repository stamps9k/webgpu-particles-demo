...
var shader_sets_query_string = `SELECT
...
var all_shader_configs_query_string = `SELECT
	shader_sets.name AS shader_set_name,
...
var shader_configs_query_string = `SELECT
	shader_configs.shader_config_id AS shader_config_id, 
	shader_configs.name AS shader_config_name, 
	shader_configs.display_name AS shader_config_display_name,
	shader_configs.validation_expression AS shader_config_validation_expression,
...
var shader_config_selection_items_query_string = `SELECT
...
const shader_sets_query_promise = () => {
  return new Promise((resolve, reject) => {
    logger_api["super_verbose_api_db"](
      "Running query " + shader_set_shaders_query_string + "...",
    );
...
const all_shader_configs_query_promise = () => {
  return new Promise((resolve, reject) => {
    logger_api["super_verbose_api_db"](
      "Running query " + shader_configs_query_string + "...",
    );
...
const shader_config_selection_items_query_promise = (shader_id: string) => {
  return new Promise((resolve, reject) => {
    logger_api["super_verbose_api_db"](
      "Running query " + shader_config_selection_items_query_string + "...",
    );
    const results = db
      .prepare(shader_config_selection_items_query_string)
      .all(shader_id);
    logger_api["super_verbose_api_db"]("... query completed.");
    resolve(results);
  });
...
const shader_set_shaders_query_promise = (shader_set_name: string) => {
  return new Promise((resolve, reject) => {
    ...
    const results = db
      .prepare(shader_set_shaders_query_string)
      .all(shader_set_name);
    logger_api["super_verbose_api_db"]("... query completed.");
    resolve(results);
  });
...
models_routes.get("/api/shader-configs", async (req, res) => {
  ...
  try {
    if (req.query.shader_set == null || req.query.shader_set == undefined) {
      ...
      logger_api["info_api_db"]("...database query complete.");
      logger_api["super_verbose_api_db"](
        "Returning " + JSON.stringify(message),
      );
      res.json({ success: true, message });
    } else {
      logger_api["info_api_db"]("Querying database...");
      var message = await shader_configs_query_promise(
        req.query.shader_set as string,
      );
      logger_api["info_api_db"]("...database query complete.");
      logger_api["super_verbose_api_db"](
        "Returning " + JSON.stringify(message),
...
models_routes.get("/api/shader-config-selection-items", async (req, res) => {
  if (
    req.query.shader_config_id == null ||
    req.query.shader_config_id == undefined
  ) {
  ...
  try {
    logger_api["info_api_db"]("Querying database...");
    var message =
      await shader_config_selection_items_query_promise(shader_config_id);
    logger_api["info_api_db"]("...database query complete.");
    logger_api["super_verbose_api_db"]("Returning " + JSON.stringify(message));
    res.json({ success: true, message });
  } catch (err) {
